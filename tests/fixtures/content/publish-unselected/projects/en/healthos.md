...
The homepage and services screenshots were captured from the frontend running locally. Health content in the interface is application sample data.
...
The related readiness lab now records four assistant-run fixture trials. It gives a concrete example of a running process returning an unsuccessful readiness response; it does not certify the HealthOS configuration or replace a backend run.
